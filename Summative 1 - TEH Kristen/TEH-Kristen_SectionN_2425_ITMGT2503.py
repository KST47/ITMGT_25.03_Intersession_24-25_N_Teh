...
for line_number, script_line in jsondata.items():
    script_string = str(script_line)
    capitalized_script_string = script_string.upper()
    new_line = capitalized_script_string
    for tag in tags:
        new_line = new_line.replace(tag,"")
    for char in unnecessary_punctuation:
        new_line = new_line.replace(char," ")
    jsondata[line_number] = new_line
...
script_word_list = []
for new_line in jsondata.values():
    final_script_words_list = new_line.split()
    for word in final_script_words_list:
        if len(word) >= 3:
            script_word_list.append(word)

wordcount_dictionary = {}
for word in script_word_list:
    wordcount_dictionary[word] = script_word_list.count(word)
...
for F in range(max_F + 1):
    for R in range(max_R + 1):
        for C in range(max_C + 1):
            configurations_to_test.append((F,R,C))

valid_configurations = []
valid_Z = []
for (F,R,C) in configurations_to_test:
    Z = (10 * F) + (12 * R) + (16 * C)
    gold_constraint = 1000 * F + 600 * R + 750 * C <= 18000
    lumber_constraint = 500 * F + 750 * R + 300 * C <= 12000
    food_constraint = 50 * F + 45 * R + 75 * C <= 1500
    if gold_constraint and lumber_constraint and food_constraint:
        valid_configurations.append((F,R,C))
        valid_Z.append(Z)

optimal_Z = max(valid_Z)
optimal_Z_index = valid_Z.index(optimal_Z)
optimal_configuration = valid_configurations[optimal_Z_index]
optimal_F = optimal_configuration[0]
optimal_R = optimal_configuration[1]
optimal_C = optimal_configuration[2]
...
def Collatz_winner(number_list):
    ...
    for number in number_list:
        collatz_length_list.append(Collatz(number)["collatz_length"])
        max_collatz_list.append(Collatz(number)["max_collatz"])
    length_winner_index = collatz_length_list.index(max(collatz_length_list))
    length_winner_max_collatz = max_collatz_list[length_winner_index]
    if length_winner_max_collatz == max(max_collatz_list):
        return number_list[length_winner_index]
    return None
